...
for i = 1:length(p.storage.trial);
    ...
    if ((isempty(p.storage.trial(i).Objectives.(field1))) || (isempty(p.storage.trial(i).Objectives.(field2))))
        ...
        plot(p.storage.trial(i).Objectives.(field1), p.storage.trial(i).Objectives.(field2),'kx');
...
if pltnearbest == 1
    for i = 1:pop_size;
        if p.properties(i).Pareto == 1;
            ...
            fppoints = plot(p.properties(i).(field1), p.properties(i).(field2),'m+');
...
% If desired, plot base response points
if pltbaseresponse == 1
    for i = 1:pop_size;
        bapoints = plot(p.properties(i).base_response.(field1), p.properties(i).base_response.(field2),'ro');
...
switch plt_response_options
    ...
    case '111'
        title(['Pareto Front of: ',field1, ' vs ', field2, ', with initial, old "Pareto points", progression, and "best" point']) 
        %legend([ppoints bapoints bpoint fppoints], {'Pareto Points', 'Point Progression' , '"Best" Point', 'Near Best'})
        legend([ppoints fppoints bapoints bpoint], {'Pareto Points', 'Good fitness points', 'Initial Points' , '"Best" Point'})
	case '011'
